...
- **Bottom navigation — four tabs** (`AppNavigation`, `bottomNavItems`): **Ernährung** (`Screen.Overview`, `overview` — the daily food log/overview), **Training** (`Screen.Workout`, `workout` — placeholder for now), **Statistik** (`Screen.Stats`, `stats`), **Einstellungen** (`Screen.Settings`, `settings`). Icons active = `Icons.Rounded.*`, inactive = `Icons.Outlined.*`: Ernährung = `Restaurant` (cutlery — food, not a house), Training = `FitnessCenter`, Statistik = `BarChart`, Einstellungen = `Settings`. The nav is **icon-only** — `NavigationBarItem` has no `label` (the pictograms are self-explanatory; the German label string lives on as each icon's `contentDescription` for accessibility). There is **no** standalone Goals tab — goal editing lives in the Settings hub. Any `Screen` route not in `bottomNavItems` (`AddFood`, `BarcodeScanner`) hides the bar.
