...
- **Bottom navigation — four tabs** (`AppNavigation`, `bottomNavItems`): **Ernährung** (`Screen.Overview`, `overview` — the daily food log/overview), **Training** (`Screen.Workout`, `workout` — placeholder for now), **Statistik** (`Screen.Stats`, `stats`), **Einstellungen** (`Screen.Settings`, `settings`). Icons active = `Icons.Rounded.*`, inactive = `Icons.Outlined.*`: Ernährung = `Restaurant` (cutlery — food, not a house), Training = `FitnessCenter`, Statistik = `BarChart`, Einstellungen = `Settings`. Labels use a compact single-line style (`labelMedium` at 10sp, `letterSpacing = 0`, `maxLines = 1`, `softWrap = false`) so the longest one ("Einstellungen") fits one line across four cells. There is **no** standalone Goals tab — goal editing lives in the Settings hub. Any `Screen` route not in `bottomNavItems` (`AddFood`, `BarcodeScanner`) hides the bar.
